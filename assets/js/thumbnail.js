	...
	for (let i = 0; i < candidates.length; i += 3) {
		...
		for (let j = i; j < i + 3; j++) {
			if (candidates[j]) {
				const context = {
					...candidates[j],
					pic_source: candidates[j][`thumbnail_${type}`],
				};
...
$.getJSON("assets/json/student-data.json", (context) => {
	var templateScript = Handlebars.compile(
		`{{#thumbnail data}} <div class="col-4 mb-5" data-toggle="modal" data-target="#modal_{{id}}"> <h4 class="student-id">{{id}}</h4> <img src="assets/img/student-data/{{pic_source}}" class="student-img" alt="Error loading {{id}}'s photo here" /> <h4 class="student-name">{{name}}</h4> </div> {{/thumbnail}}`
	);
